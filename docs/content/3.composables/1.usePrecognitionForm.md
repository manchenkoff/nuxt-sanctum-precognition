...
While working with a form, you can get the current state details by using `isDirty`, `processing` and `validating` fields. 
All values are stored as `Ref<boolean>` type.

- `isDirty` equals `true` if any of the form fields were changed from the original values.
